...
def orb(image2):
    ...
    gimg1 = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    gimg1 = cv2.equalizeHist(gimg1)
    gimg2 = cv2.cvtColor(image2, cv2.COLOR_BGR2GRAY)
    gimg2 = cv2.equalizeHist(gimg2)

    bf = cv2.BFMatcher(cv2.NORM_L1, crossCheck=True)
    orbObject = cv2.ORB_create(nfeatures=30000, nlevels=16)
    ...
    matches = bf.match(descriptors_1, descriptors_2)
    matches = sorted(matches, key=lambda x: x.distance)
    src_pts = np.float32([keypoints_1[m.queryIdx].pt for m in matches[:100]]).reshape(-1, 1, 2)
    dst_pts = np.float32([keypoints_2[m.trainIdx].pt for m in matches[:100]]).reshape(-1, 1, 2)
