...
def main():
    image = loadImage()
    show(image)
    videoMark('../data/mp1/movingball.mp4','../out/result1.avi')

def loadImage():
    return cv2.imread("../data/mp1/ball.png")
...
def maskClear(image):
    ...
    maskaNowa = cv2.medianBlur(maska_zam, ksize=5)
    return maskaNowa
...
def show(image):
    ...
    cv2.imshow('Obrazek HSV',hsvify(image))
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    cv2.imshow('Obrazek Maska',mask(image))
    cv2.waitKey(0)
    cv2.destroyAllWindows()
